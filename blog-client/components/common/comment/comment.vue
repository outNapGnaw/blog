    ...
    <transition name="module">
      <div class="pagination-box" v-if="comment.data.pagination.total_page > 1">
        <ul class="pagination-list" v-if="Object.is(sortMode, constants.SORT_TYPE.hot)">
          <li class="item" :key="index+1" v-for="(item, index) in comment.data.pagination.total_page">
            <a href="" 
    ...
    methods: {
      ...
      gravatar(email) {
        ...
        const gravatar_url = gravatar.url(email, { 
          // size: '96', 
          // rating: 'pg',
          // default: 'https://gravatar.outNapGnaw.me/anonymous.jpg', 
          protocol: 'https'
        });
        //TODO 获取头像
        // return gravatar_url.replace('https://s.gravatar.com/avatar', 'peytonblogapi.aitboy.cn/gravatar')
